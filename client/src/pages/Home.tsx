...
import { nextMobileOpenState, reportSuccessMessage, requirementTarget, signedOutMode } from "@/lib/workspaceActions";
import { MapView } from "@/components/Map";
import { LaserCollection } from "@/components/LaserCollectionFrame";
import { createTopDockController } from "@/threeui/shaders/animated-top-dock/topDockController";
...
const requirementChecklist: RequirementItem[] = [
  { id: "email-upload", module: "01", title: "Email upload (.eml and .msg)", detail: "Secure .eml and Outlook .msg upload, evidence storage, local parsing, and case creation are available. Both formats use the same private evidence boundary.", status: "available", action: "analyzer" },
  ...
  { id: "geolocation", module: "06", title: "IP geolocation and threat map", detail: "Available with explicit analyst approval. A public extracted source IP is sent to the configured IPGeolocation.io lookup service, then its approximate result is saved privately and shown on the map. Private IPs are rejected.", status: "available", action: "geolocation" },
  ...
  { id: "multi-location-map", module: "BONUS", title: "Multi-location threat map", detail: "Available when approved IP lookups have saved more than one real location. It renders private marker pins and fits the map to the signed-in analyst's completed-case coordinates.", status: "available", action: "geolocation" },
...
function SignalMark({ className = "brand-mark" }: { className?: string }) {
  return <svg className={className} viewBox="0 0 42 42" role="img" aria-label="Signal Furnace intelligence mark"><circle cx="21" cy="21" r="17" fill="none" stroke="currentColor" strokeWidth="1.5" opacity=".35" /><path d="M21 7v10M21 25v10M7 21h10M25 21h10" stroke="currentColor" strokeWidth="1.5" /><circle cx="21" cy="21" r="5" fill="currentColor" /><path d="M12 30c2.5 2.2 5.6 3.5 9 3.5s6.5-1.3 9-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" /></svg>;
}
  ...
  return (
    <main className="landing-shell">
      ...
      <section className="hero-section">
        <div className="hero-matrix" aria-hidden="true"><LaserCollection variant="matrix-field" speed={1} size={1} length={1} density={1} opacity={1} hue={0} saturation={1} brightness={1} /></div>
      ...
      <section className="evidence-section section-wrap">
        <div className="evidence-visual" role="img" aria-label="Saved email details"><img src="/manus-storage/signal-furnace-evidence_b9c4e457.jpg" alt="Saved email details" onError={(event) => { event.currentTarget.style.display = "none"; }} /><div className="evidence-visual__fallback" aria-hidden="true"><span>RFC822</span><strong>PRIVATE EVIDENCE</strong><i /><b>SHA-256 · HEADER · IOC</b></div><div className="evidence-visual__caption"><span>DETAILS SAVED</span><strong>FILE CHECK</strong><i /></div></div>
      ...
      <section id="team" className="team-section section-wrap">
        ...
        <div className="team-list">
          <article className="team-profile">
            <span>01</span>
            <div>
              <strong>Vishalkumaran V</strong><p>Team Leader · Backend Developer</p>
          ...
          {[["Srinidhi H", "Backend Developer", "https://srinidhi-ai-gwygmuea.manus.space/"], ["Sayasree T K", "R&D Engineer", "https://sayasree-eee-mk93ir2n.manus.space/?code=3keuGTTM4i7Lav9Cwts4TL"], ["Surya V", "Frontend Developer", "https://suryavijay0107-star.github.io/playful-geometric-studio/#experience"], ["Rohini S", "Frontend Developer", "https://rohinifolio-mmckdlnu.manus.space/"], ["Sankarprasath S", "Presenter", "https://sankarfolio-rydjgeka.manus.space/"]].map(([member, role, portfolio], index) => <a key={member} className="team-member" href={portfolio} target="_blank" rel="noreferrer"><span>0{index + 2}</span><div><strong>{member}</strong><small>{role}</small></div><ArrowUpRight size={15} /></a>)}
        </div>
      </section>

      <footer className="landing-footer"><BrandMark /><p>A simple tool for checking suspicious emails.</p><a href="/docs" className="landing-footer__docs">Security docs <ChevronRight size={15} /></a><button onClick={() => launchWorkspace("dashboard")}>Open dashboard <ArrowUpRight size={15} /></button></footer>
    </main>
  );
}
...
function WorkspaceSidebar({ activeView, setActiveView, compact, setCompact, userName, signOut }: { activeView: View; setActiveView: (view: View) => void; compact: boolean; setCompact: (compact: boolean) => void; userName: string; signOut: () => void }) {
  return (
    <aside className={`workspace-sidebar ${compact ? "workspace-sidebar--compact" : ""}`}>
      <div className="sidebar-brand"><div className="terminal-brand"><SignalMark className="terminal-brand__mark" /><div><strong>Signal Furnace</strong><small>Private security workspace</small></div></div><button aria-label="Toggle sidebar" onClick={() => setCompact(!compact)}>{compact ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}</button></div>
...
function WorkspaceTopDock({ activeView, setActiveView, userName, signOut }: { activeView: View; setActiveView: (view: View) => void; userName: string; signOut: () => void }) {
  const dockRef = useRef<HTMLElement>(null);
  useEffect(() => {
    const dock = dockRef.current;
    if (!dock) return undefined;
    return createTopDockController(dock, () => ({ proximity: 122, spring: 0.19, damping: 0.70, widthGrowth: 17, heightGrowth: 16, drop: 3.5, lockTrack: true }));
  }, []);

  return <header className="workspace-top-dock"><div className="workspace-top-dock__brand"><SignalMark className="workspace-top-dock__mark" /><div><strong>Signal Furnace</strong><small>Private investigation workspace</small></div></div><nav ref={dockRef} className="workspace-top-dock__nav" aria-label="Workspace sections" data-dock-state="idle" data-dock-max="0.00">{navItems.map(({ id, label, icon: Icon }) => <button key={id} type="button" data-dock-item aria-current={activeView === id ? "page" : undefined} className={activeView === id ? "active" : ""} onClick={() => setActiveView(id)}><Icon size={16} /><span>{label}</span></button>)}</nav><div className="workspace-top-dock__actions"><button title="Sign out" onClick={signOut}><LogOut size={17} /><span>{userName.slice(0, 1).toUpperCase()}</span></button><Button onClick={() => setActiveView("analyzer")} className="new-case"><Plus size={17} /> New case</Button></div></header>;
}

function Topbar({ activeView, setActiveView, setMobileOpen }: { activeView: View; setActiveView: (view: View) => void; setMobileOpen: (value: boolean) => void }) {
  const current = navItems.find((item) => item.id === activeView)!;
  return <header className="workspace-topbar"><button className="mobile-menu" aria-label="Open menu" onClick={() => setMobileOpen(true)}><Menu size={20} /></button><div className="breadcrumb"><span>WORKSPACE</span><ChevronRight size={14} /><strong>{current.label}</strong></div><div className="topbar-actions"><ThemeToggle compact /><button title="Open AI help" onClick={() => setActiveView("assistant")} className="guide-trigger"><Bot size={18} /></button><button title="Filter results" onClick={() => toast.info("Filters need live data.")}><ListFilter size={18} /></button><button title="Notifications" onClick={() => toast.info("There are no live notifications yet.")} className="notification-button"><Activity size={18} /><i /></button><Button onClick={() => setActiveView("analyzer")} className="new-case"><Plus size={17} /> New case</Button></div></header>;
}
...
function AnalyzerView({ setActiveView, selectInvestigation }: { setActiveView: (view: View) => void; selectInvestigation: (id: number) => void }) {
  ...
  const [lastResult, setLastResult] = useState<{ investigationId: number; caseNumber: string; ownerAlert: "not_needed" | "delivered" | "unavailable"; parsed: { sender: string | null; recipient: string | null; originatingIp: string | null; indicators: Array<{ type: string; value: string; source: string }>; threatScore: number; confidence: number; severity: string; summary: string; reasons: string[]; findings: Array<{ kind: string; severity: string; value: string; detail: string }>; ai: { category: string; confidence: number; socialEngineering: string; recommendations: string[] } | null } } | null>(null);
  ...
  const onFile = (event: ChangeEvent<HTMLInputElement>) => { const chosen = event.target.files?.[0]; if (!chosen) return; const name = chosen.name.toLowerCase(); if (!name.endsWith(".eml") && !name.endsWith(".msg")) { toast.error("Upload an .eml or .msg email file."); event.target.value = ""; return; } if (chosen.size > 4 * 1024 * 1024) { toast.error("Email files must be smaller than 4 MB."); event.target.value = ""; return; } setFile(chosen); setLastResult(null); toast.info(`${chosen.name} is ready for a secure server-side check.`); };
  const analyze = async () => { if (!file || upload.isPending) { if (!file) toast.info("Choose an .eml or .msg file first."); return; } const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onerror = () => reject(new Error("The file could not be read.")); reader.onload = () => resolve(String(reader.result)); reader.readAsDataURL(file); }); const isMsg = file.name.toLowerCase().endsWith(".msg"); upload.mutate({ filename: file.name, mimeType: file.type || (isMsg ? "application/vnd.ms-outlook" : "message/rfc822"), base64: dataUrl.split(",")[1] || "" }); };
  return <div className="page-content analyzer-view">
    <div className="page-title-row"><div><SectionLabel>Start here</SectionLabel><h1>Check an <span>email.</span></h1><p>Upload an .eml or .msg email to check its sender, headers, links, and attachment names.</p></div><div className="file-formats"><FileText size={17} /><span>FILES</span><b>.EML</b><b>.MSG</b></div></div>
    <div className="analyzer-layout"><section className="upload-panel"><div className="upload-panel__grid" /><input ref={input} onChange={onFile} type="file" accept=".eml,.msg,message/rfc822,application/vnd.ms-outlook" className="sr-only" /><div className="upload-icon"><Upload size={27} /></div><h2>Add an email file.</h2><p>When you start the check, the file is uploaded to protected evidence storage and a private case is created for your account.</p><Button onClick={chooseFile} className="hero-primary">Choose email file <Upload size={17} /></Button><button className="upload-drop" onClick={chooseFile}>or choose a file here</button>{file && <div className="staged-file"><FileText size={18} /><span><strong>{file.name}</strong><small>{Math.ceil(file.size / 1024)} KB · ready for secure upload</small></span><button onClick={() => { setFile(null); setLastResult(null); if (input.current) input.current.value = ""; }}><X size={16} /></button></div>}</section><aside className="analysis-guide"><span>CHECK STATUS</span><ol><li><i>01</i><div><strong>Choose an .eml or .msg file</strong><p>The maximum file size is 4 MB.</p></div></li><li><i>02</i><div><strong>Run the evidence check</strong><p>Headers, links, attachment names, IPs, spoofing signals, and a bounded content review run on the server.</p></div></li><li><i>03</i><div><strong>Review the saved case</strong><p>Evidence, IOCs, a timeline, and notes stay in your signed-in workspace.</p></div></li></ol><Button onClick={analyze} disabled={!file || upload.isPending} className="analyze-button"><ScanSearch size={17} /> {upload.isPending ? "Checking email…" : "Start secure check"}</Button></aside></div><section className="analysis-result"><div className="analysis-result__head"><div><span>EMAIL CHECK RESULT</span><h2>{lastResult ? `Saved as ${lastResult.caseNumber}` : "No result yet"}</h2></div>{lastResult && <Button onClick={() => setActiveView("forensics")} className="outline-cta">Open case details <ChevronRight size={16} /></Button>}</div>{lastResult ? <div className="analysis-result__body"><div className="analysis-result__summary"><SeverityBadge value={lastResult.parsed.severity} /><strong>Score {lastResult.parsed.threatScore}/100 · {lastResult.parsed.confidence}% confidence</strong><p>{lastResult.parsed.summary}</p></div><div className="evidence-signal-grid"><div><span>FROM DOMAIN</span><strong>{emailDomain(lastResult.parsed.sender) || "Not found"}</strong><small>{lastResult.parsed.sender || "No sender address was present in trusted headers."}</small></div><div><span>TO DOMAIN</span><strong>{emailDomain(lastResult.parsed.recipient) || "Not found"}</strong><small>{lastResult.parsed.recipient || "No recipient address was present in trusted headers."}</small></div><div><span>ORIGINATING IP</span><strong>{lastResult.parsed.originatingIp || "Not found"}</strong><small>{lastResult.parsed.originatingIp ? "Eligible only after public-IP validation." : "Email domains are not IP addresses and cannot be geolocated as sender devices."}</small></div></div>{!lastResult.parsed.originatingIp && <div className="no-ip-callout"><Globe2 size={18} /><div><strong>No originating IP was found in the trusted headers.</strong><p>This email can still be reviewed through its sender domain, links, authentication results, and message structure. A map lookup needs a real public IP from a Received header; it must not be guessed from gmail.com or another email domain.</p></div></div>}{lastResult.parsed.ai && <p className="analysis-pending"><strong>AI category: {lastResult.parsed.ai.category.replace(/_/g, " ")} · {lastResult.parsed.ai.confidence}% confidence</strong><br />{lastResult.parsed.ai.socialEngineering || "No social-engineering pattern was stated."}<br />Next step: {lastResult.parsed.ai.recommendations[0] || "Review the saved evidence."}</p>}<p className="analysis-pending">{lastResult.parsed.reasons.length ? `Signals: ${lastResult.parsed.reasons.join("; ")}.` : "No structural warning was found."}</p></div> : <p className="analysis-pending">No example result is shown. Start a secure check to upload a real .eml or .msg file and save the result.</p>}</section>
  </div>;
}
...
function GeolocationCaseFlow() {
  ...
  const senderDomain = emailDomain(detail.data?.artifact?.sender);
  const recipientDomain = emailDomain(detail.data?.artifact?.recipient);
  const onMapReady = useCallback((map: google.maps.Map) => {
    const maps = window.google?.maps;
  ...
  return <div className="page-content geolocation-view"><div className="page-title-row"><div><SectionLabel>Location map</SectionLabel><h1>Where alerts <span>may come from.</span></h1><p>Use a completed case with an extracted public source IP. The result may identify an ISP, mail relay, or network point rather than the sender or device, and is never exact device tracking.</p></div><span className="time-chip"><MapPin size={15} /> {saved.length ? `${saved.length} saved` : "Step 1: choose a case"}</span></div>{!cases.data?.length ? <section className="panel empty-panel"><EmptyData icon={MapPin} title="Create a case before opening the map" copy="Step 1: check an .eml email. Step 2: return here and choose that private case. Step 3: approve an approximate location lookup only if a public source IP was extracted." /></section> : <><section className="panel"><div className="panel-header"><div><span>STEP 1 — SELECT A CASE</span><h2>Choose private evidence</h2></div></div><div className="simple-record-list">{cases.data.map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)}><SeverityBadge value={item.severity} /><strong>{item.caseNumber}</strong><span>{item.title}</span></button>)}</div>{detail.data && <div className="analysis-pending"><strong>{sourceIp ? `Step 2 — extracted source IP: ${sourceIp}` : "No usable public source IP was extracted from this case."}</strong><p>{sourceIp ? "Step 3 — approve this lookup to send only the public IP to ipwho.is. The approximate country, region, city, and map coordinate stay in your private workspace." : "The map cannot run without a source IP found in the uploaded email headers. Private, loopback, and link-local IPs are blocked before any lookup."}</p>{!sourceIp && <div className="no-ip-callout"><Globe2 size={18} /><div><strong>No map-ready IP in this case.</strong><p>{senderDomain || recipientDomain ? `Email domain context: ${[senderDomain && `from ${senderDomain}`, recipientDomain && `to ${recipientDomain}`].filter(Boolean).join(" · ")}. A domain identifies a mail service or organization, not a device location.` : "This case has no sender or recipient domain available either."}</p></div></div>}<Button className="outline-cta" disabled={!sourceIp || enrich.isPending} onClick={() => enrich.mutate({ investigationId: detail.data.investigation.id })}><MapPin size={15} /> {enrich.isPending ? "Looking up…" : "Approve location lookup"}</Button></div>}</section>{saved.length ? <section className="panel" style={{ marginTop: 14 }}><div className="panel-header"><div><span>PRIVATE THREAT MAP</span><h2>Approved approximate locations</h2></div><Button className="outline-cta" onClick={() => void locations.refetch()} disabled={locations.isFetching}><RefreshCw size={15} className={locations.isFetching ? "animate-spin" : ""} /> {locations.isFetching ? "Refreshing…" : "Refresh map"}</Button></div><p className="map-live-note">Live refresh checks your saved approved results every 30 seconds. It does not track a device or request a new provider lookup.</p><MapView className="w-full h-[390px]" initialCenter={{ lat: saved[0].latitude || 0, lng: saved[0].longitude || 0 }} initialZoom={saved.length > 1 ? 2 : 5} onMapReady={onMapReady} /><div className="simple-record-list">{saved.map((item) => <div key={item.id}><MapPin size={15} /><strong>{item.ip}</strong><span>{[item.city, item.region, item.country].filter(Boolean).join(", ") || "Approximate location not named"} · {new Date(item.enrichedAt).toLocaleString()}</span></div>)}</div></section> : null}</>}</div>;
}
...
function AssistantView({ navigate }: { navigate: (view: View) => void }) {
  const history = trpc.guide.history.useQuery();
  const utils = trpc.useUtils();
  const assistant = trpc.guide.ask.useMutation({ onSuccess: ({ navigateTo }) => { void utils.guide.history.invalidate(); if (navigateTo) toast.info(`Relevant page: ${navItems.find((item) => item.id === navigateTo)?.label}.`); }, onError: (error) => toast.error(error.message) });
  const messages: ChatMessage[] = history.data?.map((message) => ({ role: message.role, content: message.content })) || [{ role: "assistant", content: "I am the SIH26106 Requirements Assistant. I can explain only the required features, safety limits, evidence workflow, and available integrations. I cannot inspect or change your private case data." }];
  return <div className="page-content assistant-view requirements-assistant-view"><div className="assistant-heading"><div className="assistant-orb"><Bot size={32} /></div><SectionLabel>AI requirements assistant</SectionLabel><h1>Ask about the<br /><span>required workflow.</span></h1><p>Private to your signed-in account. This assistant answers SIH26106 and Signal Furnace requirement questions only; it cannot read evidence or perform actions.</p></div><section className="requirements-chat-shell"><div className="requirements-chat-shell__scope"><ShieldCheck size={16} /><span>Requirements only · no case access · no settings changes · no external actions</span></div><AIChatBox messages={messages} onSendMessage={(question) => assistant.mutate({ question, currentView: "assistant" })} isLoading={assistant.isPending} height="min(60vh, 640px)" emptyStateMessage="Ask about requirements, evidence, attachments, maps, reports, campaigns, or privacy." placeholder="Ask a SIH26106 requirement question…" suggestedPrompts={["What is the secure email evidence workflow?", "How does attachment risk analysis work?", "What does approximate IP location mean?", "How are campaigns identified from IOCs?", "What is included in a case report?"]} /></section></div>;
}

function ReportsView() { const cases = trpc.analysis.list.useQuery(); const [selectedId, setSelectedId] = useState<number | null>(null); const activeId = selectedId || cases.data?.[0]?.id; const detail = trpc.analysis.detail.useQuery({ investigationId: activeId || 0 }, { enabled: Boolean(activeId) }); const exportReport = (format: "csv" | "pdf") => { if (!detail.data) return; if (format === "csv") downloadCaseCsv(detail.data); else downloadCasePdf(detail.data); toast.success(reportSuccessMessage(format)); }; if (!cases.data?.length && !cases.isLoading) return <div className="page-content reports-view"><div className="page-title-row"><div><SectionLabel>Reports</SectionLabel><h1>Share a <span>clear result.</span></h1><p>Reports use only a completed case and its saved evidence.</p></div><span className="time-chip"><FileText size={15} /> No reports</span></div><section className="panel empty-panel"><EmptyData icon={FileText} title="No reports yet" copy="Check an .eml email first. You can then create a real CSV or PDF report from that private case." /></section></div>; return <div className="page-content reports-view"><div className="page-title-row"><div><SectionLabel>Reports</SectionLabel><h1>Share a <span>clear result.</span></h1><p>Export a real CSV or PDF from one of your saved private cases. No example reports are created.</p></div><span className="time-chip"><FileText size={15} /> {cases.data?.length || 0} cases</span></div><section className="panel"><div className="panel-header"><div><span>SELECT CASE</span><h2>Saved investigation evidence</h2></div></div><div className="simple-record-list">{cases.data?.map((item) => <button onClick={() => setSelectedId(item.id)} key={item.id}><SeverityBadge value={item.severity} /><strong>{item.caseNumber}</strong><span>{item.title} · score {item.threatScore}/100</span></button>)}</div></section>{detail.data && <section className="panel" style={{ marginTop: 14 }}><div className="panel-header"><div><span>EXPORT READY</span><h2>{detail.data.investigation.caseNumber}</h2></div><SeverityBadge value={detail.data.investigation.severity} /></div><div className="simple-record-list"><div><FileText size={17} /><strong>{detail.data.investigation.title}</strong><span>Includes saved score, evidence metadata, AI review when present, IOCs, timeline, and analyst notes.</span><Button onClick={() => exportReport("csv")} className="outline-cta"><Download size={15} /> CSV</Button><Button onClick={() => exportReport("pdf")} className="outline-cta"><Download size={15} /> PDF</Button></div></div></section>}</div>; }
...
function SettingsView() {
  const { user } = useAuth();
  const team = trpc.admin.users.useQuery(undefined, { enabled: user?.role === "admin" });
  const auditEvents = trpc.admin.auditEvents.useQuery(undefined, { enabled: user?.role === "admin" });
  const utils = trpc.useUtils();
  const updateRole = trpc.admin.updateUserRole.useMutation({ onSuccess: () => { utils.admin.users.invalidate(); toast.success("User role updated."); }, onError: (error) => toast.error(error.message) });
  return <div className="page-content settings-view"><div className="page-title-row"><div><SectionLabel>Settings</SectionLabel><h1>Website <span>settings.</span></h1><p>Review signed-in access and analyst-approved data services.</p></div><span className="time-chip"><Database size={15} /> {user?.role === "admin" ? "Administrator" : "Analyst access"}</span></div><div className="settings-grid">{user?.role === "admin" && <section className="panel settings-panel settings-panel--wide"><div className="panel-header"><div><span>SECURITY AUDIT</span><h2>Durable access events</h2></div><ShieldCheck size={18} /></div>{auditEvents.data?.length ? <div className="simple-record-list">{auditEvents.data.map((event) => <div key={event.id}><ShieldCheck size={15} /><strong>{event.eventType}</strong><span>{event.resourceType ? `${event.resourceType}${event.resourceId ? ` · ${event.resourceId}` : ""}` : "System event"} · {event.actorRole} · {new Date(event.createdAt).toLocaleString()}</span></div>)}</div> : <EmptyData icon={ShieldCheck} title="No audit events yet" copy="Durable security events appear after protected case, role, or ingestion actions." compact />}</section>}<section className="panel settings-panel"><div className="panel-header"><div><span>TEAM ACCESS</span><h2>Signed-in users and roles</h2></div><ShieldCheck size={18} /></div>{user?.role === "admin" ? team.data?.length ? <div className="simple-record-list">{team.data.map((member) => <div key={member.id}><BriefcaseBusiness size={15} /><strong>{member.name || member.email || `User ${member.id}`}</strong><span>{member.email || "No email shared"} · last sign-in {new Date(member.lastSignedIn).toLocaleString()}</span><label className="case-status-control">Role <select value={member.role} disabled={updateRole.isPending || member.id === user.id} onChange={(event) => updateRole.mutate({ userId: member.id, role: event.target.value as "user" | "admin" })}><option value="user">User</option><option value="admin">Administrator</option></select></label></div>)}</div> : <EmptyData icon={BriefcaseBusiness} title="No signed-in users yet" copy="A user appears here after completing secure sign-in." compact /> : <EmptyData icon={ShieldCheck} title="Administrator access required" copy="Only administrators can view the team list or change roles." compact />}</section><section className="panel settings-panel"><div className="panel-header"><div><span>DATA SOURCES</span><h2>Approved provider checks</h2></div><Network size={18} /></div><div className="policy-grid"><p><i>01</i><span><strong>Approximate IP location</strong>Ready after an analyst approves a stored public source-IP lookup. Private addresses are blocked.</span><b>READY</b></p><p><i>02</i><span><strong>AbuseIPDB and VirusTotal</strong>Ready for separate analyst-approved public-IP reputation checks. Results stay with the private case and its timeline.</span><b>READY</b></p><p><i>03</i><span><strong>PhishTank public feed</strong>Ready for analyst-approved checks of extracted URLs against the verified-online HTTPS feed. A four-hour server cache limits downloads without an API key.</span><b>READY</b></p><p><i>04</i><span><strong>High-risk owner alerts</strong>A notification is attempted when a completed check scores 60 or higher. Its delivery result is recorded in the case timeline.</span><b>READY</b></p></div></section><section className="panel settings-panel settings-panel--wide"><div className="panel-header"><div><span>SAFETY RULES</span><h2>Data handling</h2></div><ShieldCheck size={18} /></div><div className="policy-grid"><p><i>01</i><span><strong>Do not show made-up results</strong>Only completed checks from uploaded evidence appear in the workspace.</span><b>ON</b></p><p><i>02</i><span><strong>Secure evidence upload</strong>An .eml file is uploaded only when a signed-in analyst starts a check, then stored as private evidence.</span><b>ON</b></p><p><i>03</i><span><strong>Keep the AI Guide limited</strong>The Guide can explain pages and open approved screens only. It cannot access or change case data.</span><b>ON</b></p></div></section></div></div>;
}
...
function Workspace({ initialView, userName, signOut }: { initialView: View; userName: string; signOut: () => void }) {
  ...
  const renderView = () => { switch (activeView) { case "dashboard": return <DashboardView setActiveView={setActiveView} selectInvestigation={setSelectedInvestigationId} />; case "analyzer": return <AnalyzerView setActiveView={setActiveView} selectInvestigation={setSelectedInvestigationId} />; case "intelligence": return <InvestigationWorkbench />; case "geolocation": return <GeoLocationScreen />; case "forensics": return <ForensicsViewWithAiRetry selectedInvestigationId={selectedInvestigationId} selectInvestigation={setSelectedInvestigationId} />; case "assistant": return <AssistantView navigate={setActiveView} />; case "reports": return <ReportsView />; case "settings": return <SettingsView />; case "requirements": return <RequirementsView navigate={setActiveView} />; } };
  return <div className="workspace-shell workspace-shell--top-dock"><WorkspaceTopDock activeView={activeView} setActiveView={setActiveView} userName={userName} signOut={signOut} /><div className="workspace-main"><Topbar activeView={activeView} setActiveView={setActiveView} setMobileOpen={setMobileOpen} />{renderView()}</div>{mobileOpen && <div className="mobile-nav-overlay"><div className="mobile-nav-head"><div className="terminal-brand"><SignalMark className="terminal-brand__mark" /><div><strong>Signal Furnace</strong><small>Private security workspace</small></div></div><button aria-label="Close navigation" onClick={() => setMobileOpen(false)}><X size={20} /></button></div><nav>{navItems.map(({ id, label, icon: Icon }) => <button onClick={() => { setActiveView(id); setMobileOpen(false); }} className={activeView === id ? "active" : ""} key={id}><Icon size={18} />{label}</button>)}</nav><button onClick={signOut} className="mobile-exit"><LogOut size={18} /> Sign out</button></div>}</div>;
}
...
export default function Home() {
  ...
  const [requestedView, setRequestedView] = useState<View>(() => previewMode === "requirements" ? "requirements" : previewMode === "guide" ? "assistant" : previewMode === "geolocation" ? "geolocation" : previewMode === "intelligence" ? "intelligence" : "dashboard");
  useEffect(() => {
  ...
  const signOut = async () => { await logout(); setMode(signedOutMode()); window.scrollTo({ top: 0, behavior: "smooth" }); toast.success("You have signed out."); };
