...
| Attachment checks | Implemented with bounded enrichment | Local filename, extension, double-extension, MIME-consistency, size, and SHA-256 checks are persisted; an analyst may approve a per-hash VirusTotal check | No attachment bytes are sent automatically, and no malware execution or sandbox verdict is claimed |
...
| IP geolocation and map | Implemented conditionally | Analyst approval, public-IP eligibility gate, encrypted IPinfo/IPGeolocation.io corroboration, 24-hour cache, private location storage, provenance/status display, and supported marker-based multi-location map | Requires a real publicly routable source IP; the result is approximate network location, not device tracking; private, reserved, and RFC 5737 documentation IPs are blocked |
...
| Evidence and timeline | Implemented | Uploaded evidence metadata, structural analysis events, provider events, AI review events, analyst notes, and a private tamper-evident sequential evidence chain are persisted | The chain is a private hash-linked record, not a public blockchain; raw evidence is never replaced by fabricated test data |
...
| AI cyber assistant | Implemented with a narrow scope | Private per-analyst requirements chat answers SIH26106 and application requirement questions, with suggested workflow questions and approved navigation | It refuses unrelated general questions and cannot read case content, upload, modify cases, send data, or access account controls |
| Reports | Implemented | Real CSV and PDF exports include stored evidence metadata, scores, AI review, IOCs, attachment-risk analysis, evidence-chain records, campaign correlation, timeline events, provider evidence, and notes | Reports contain only data saved for the signed-in analyst |
...
| Similar attack detection | Implemented | Private IOC-based campaign correlation groups cases that share real URLs, domains, IPs, or email indicators, and shows only evidence-backed memberships | Comparisons are limited to the signed-in analyst’s saved cases; no association is fabricated |
| Multi-location threat map | Implemented conditionally | Private marker pins and automatic viewport fitting are shown when more than one approved real location exists | It needs more than one approved real saved location; the deprecated Google Heatmap Layer is not used |
...
The final deterministic validation covered source-IP extraction, oldest eligible `Received`-hop selection, domain-versus-IP safety, shared button-action behavior, security controls, requirements-assistant limits, attachment-risk behavior, provider boundaries, and progressive-workspace controls. The full Vitest suite, TypeScript checking, and production build passed. The build still reports a non-blocking large-bundle warning.

## Remaining work before claiming complete production parity

The following items are not failures of the core MVP, but they should remain clearly described as conditional or pending: `.msg` parsing; live DNS and domain reputation; malware execution or sandbox scanning; a correctly formatted authorized email containing a real public source IP for map, AbuseIPDB, VirusTotal, and encrypted corroboration happy-path validation; a real score of at least 60 for owner-alert validation; an administrator-session check; a future owned case that genuinely lacks AI output for the retry control; and a fresh authenticated browser check of theme persistence and the populated post-redesign case/intelligence layouts.
